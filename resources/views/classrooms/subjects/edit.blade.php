      ...
      <form action="{{ route('classrooms.subjects.update', ['subject' => $subject->id]) }}" method="POST">
        ...
        <div class="grid grid-cols-1 gap-6 mt-4">
          <div>
            ...
            <x-input type="text" name="name" id="name" value="{{ old('name', $subject->name) }}" required />
          </div>
          ...
          <div>
            ...
            <select name="classroom_id" id="classroom_id"
              ...
              @foreach ($classrooms as $classroom)
                <option @selected(old('classroom_id', $subject->classroom_id) === $classroom->id) value="{{ $classroom->id }}">{{ $classroom->grade }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div>
            ...
            <select name="teacher_id" id="teacher_id"
              ...
              @foreach ($teachers as $teacher)
                <option @selected(old('teacher_id', $subject->teacher_id) === $teacher->id) value="{{ $teacher->id }}">{{ $teacher->name }}</option>
              @endforeach
            </select>
          </div>
        </div>
